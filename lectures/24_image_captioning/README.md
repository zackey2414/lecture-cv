...
画像分類は「1024 個のラベルから1つ選ぶ」タスクでした。キャプションは違います。**任意の自然文を1語ずつ生成する**ので、出力空間は「語彙サイズ ^ 文長」という天文学的な広さになります。だからキャプションモデルは、内部に必ず2つの部品を持ちます。
...
つまりキャプションは「画像を読む CV」と「文を書く NLP」の合流点です。本章で使う3モデルは、この2部品の**つなぎ方**が違うだけ、と捉えると一気に整理できます。直感的には「**画像を見ながら文章を書く穴埋め**」で、デコーダは毎ステップ「ここまで書いた文 ＋ 画像」を条件に次の語を確率で選びます。
...
BLIP と ViT-GPT2 は **Encoder–Decoder 型**で、デコーダの各層が cross-attention を通じて画像を「見ながら」書きます。GIT は **デコーダのみ（decoder-only）型**で、画像をパッチトークン化して文章トークンの前に置き、ふつうの言語モデルとして続きを生成します。BLIP-2 や最近の VLM（第25回）はこの decoder-only 方向の発展形です。

実装上の重要な差は **前処理器（processor）**です。BLIP と GIT は `AutoProcessor` 一つで「画像＋テキストをまとめて」前処理できます。ところが ViT-GPT2 の `AutoProcessor` は**中身が GPT2Tokenizer だけ**で画像を渡せません。ViT-GPT2 は `AutoImageProcessor`（画像）＋ `AutoTokenizer`（文字）を**別々に**使う必要があります。本章の `caption_helpers.Captioner` は、この差を `kind="combined" / "encdec"` で吸収し、どのモデルでも `caption_images(cap, images, ...)` の同じ呼び方で使えるようにしてあります。
