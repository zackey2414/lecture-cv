...
Computer Vision（CV）を「**AI の補助なしでも自分一人でゴリゴリ書け、内容を熟知している**」状態まで叩き込むための、ハンズオン講座リポジトリです。OpenCV / Pillow の画像基礎から、古典 CV、深層 CV（分類・検出・セグメンテーション・深度/姿勢/追跡）、CLIP/SigLIP とベクトル検索（FAISS）、キャプション/VQA/OCR、生成・編集、各タスクの**精度評価**（物体検出 mAP は自力実装）、そして量子化・枝刈り・ONNX・知識蒸留などの**モデル圧縮/高速化**まで、全 45 モジュールを段階的に実装します。
...
│   ├── 27_… 〜 29_…/         # ★作成済み（マスター水準: 深度/姿勢/フロー・追跡・動画行動認識）
│   ├── 30_…, 42_…/          # ★作成済み（顔: 検出+認識+人物クラスタリング / マルチモーダル検索）
│   ├── 31_… 〜 33_…/         # ★作成済み（生成・編集 / 異常検知・IQA / マルチモーダル埋め込み）
│   ├── 34_… 〜 37_…/         # ★作成済み（最適化: プロファイリング/量子化・枝刈り/ONNX/ランタイム）
│   ├── 38_…, 39_…/          # ★作成済み（知識蒸留 / CLIP・VLM蒸留）
│   ├── 40_…, 41_…/          # ★作成済み（Cluster-CLIP 総仕上げ＝全要素の統合再構築）
│   ├── 42_…, 44_…/          # ★作成済み（マルチモーダル検索 / 埋め込みクラスタリング）
│   └── 43_…/                # ★作成済み（色空間と調整: 明るさ/彩度/色相/RGB↔HSV ほか）
│   # ── 全45モジュール（00〜44）作成・実行検証・マスター水準 ──
├── site/                     # 教材閲覧サイト（静的HTML・生成物）→ site/index.html を開く
├── tools/build_site.py       # 閲覧サイトのビルダー（Markdown→HTML）
├── docs/roadmap.md           # 全 45 モジュールのロードマップ（必読）
