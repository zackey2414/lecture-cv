...
もう一つ実務で頻出するのが **インクリメンタル更新**です。`IndexIDMap` に `add_with_ids` で逐次追加すれば、検索可能な状態を保ったままデータを増やせます。定期的に `write_index` で永続化し、メタDBと整合させる——これが画像が流れ込み続けるストリーム処理の基本設計です（Cluster-CLIP の `stream/writer.py` がまさにこの実例で、第40・41回で扱います）。ただし IVF/PQ 系は最初に学習したセル中心が前提なので、データ分布が大きく変わったら `train` のやり直し（再構築）が必要になる点は覚えておきましょう。
