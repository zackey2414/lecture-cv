...
**`02_embedding_distillation.py` — この章の中心。** teacher の画像埋め込みを**一度だけ前計算**してキャッシュし（teacher は `eval()` + `inference_mode()` で凍結）、student を `(1 - cos) + MSE` で学習する。学習前は student と teacher の整合度が約 0.03（ランダム）なのが、約 40 エポック（CPU で数秒）で **0.99** まで上がる。teacher 画像タワー 8785 万 → student 約 16 万（**x551 圧縮**）。学習後の重みは `lectures/39_clip_distillation/outputs/student_distilled.pt` に保存される。

<figure class="lec-fig"><svg viewBox="0 0 660 290" role="img" aria-label="埋め込み蒸留の学習ループ teacherを一度だけ前計算しキャッシュ studentをループ更新" font-family="ui-sans-serif, system-ui, 'Noto Sans JP', sans-serif"><text x="330" y="28" text-anchor="middle" font-size="15" font-weight="700" fill="#18181b">埋め込み蒸留の学習ループ — teacher は一度だけ前計算</text><rect x="30" y="54" width="104" height="52" rx="7" fill="#f4f4f5" stroke="#71717a" stroke-width="2"/><text x="82" y="85" text-anchor="middle" font-size="13" font-weight="700" fill="#3f3f46">ラベル無し画像</text><line x1="134" y1="80" x2="170" y2="80" stroke="#71717a" stroke-width="2"/><polygon points="176,80 166,75 166,85" fill="#71717a"/><rect x="176" y="46" width="168" height="68" rx="7" fill="#ffedd5" stroke="#c2410c" stroke-width="2"/><text x="260" y="76" text-anchor="middle" font-size="13.5" font-weight="700" fill="#c2410c">teacher 凍結 ❄</text><text x="260" y="97" text-anchor="middle" font-size="12" fill="#52525b">512次元を前計算</text><line x1="344" y1="80" x2="384" y2="80" stroke="#71717a" stroke-width="2"/><polygon points="390,80 380,75 380,85" fill="#71717a"/><rect x="390" y="48" width="160" height="64" rx="7" fill="#dbeafe" stroke="#2563eb" stroke-width="2"/><text x="470" y="74" text-anchor="middle" font-size="13.5" font-weight="700" fill="#1d4ed8">埋め込みキャッシュ</text><text x="470" y="95" text-anchor="middle" font-size="12" fill="#52525b">一度だけ前計算</text><rect x="30" y="186" width="176" height="66" rx="7" fill="#fff7ed" stroke="#ea580c" stroke-width="2"/><text x="118" y="213" text-anchor="middle" font-size="13.5" font-weight="700" fill="#c2410c">student forward</text><text x="118" y="234" text-anchor="middle" font-size="12" fill="#52525b">64px 入力</text><line x1="206" y1="219" x2="382" y2="219" stroke="#71717a" stroke-width="2"/><polygon points="388,219 378,214 378,224" fill="#71717a"/><text x="297" y="210" text-anchor="middle" font-size="11.5" fill="#3f3f46">埋め込みを出力</text><rect x="390" y="186" width="160" height="66" rx="7" fill="#fafafa" stroke="#16a34a" stroke-width="2"/><text x="470" y="224" text-anchor="middle" font-size="13.5" font-weight="700" fill="#15803d">ロス (1−cos)+MSE</text><line x1="470" y1="112" x2="470" y2="180" stroke="#71717a" stroke-width="2"/><polygon points="470,186 465,176 475,176" fill="#71717a"/><text x="426" y="150" text-anchor="middle" font-size="11.5" fill="#c2410c">teacher 目標</text><polyline points="470,252 470,276 118,276 118,258" fill="none" stroke="#ea580c" stroke-width="2"/><polygon points="118,252 113,262 123,262" fill="#ea580c"/><text x="300" y="271" text-anchor="middle" font-size="12" font-weight="700" fill="#ea580c">勾配は student だけ・エポック繰り返し</text></svg><figcaption><b>埋め込み蒸留の学習ループ</b>です。まず <b>teacher を凍結 ❄</b> し、全画像の <b>512次元埋め込みを一度だけ前計算してキャッシュ</b>します（以降 teacher は動かしません）。あとは <b>student forward（64px）→ <code>(1−cos)+MSE</code> のロス → student のみ更新</b> をエポック分くり返すだけで、teacher 埋め込みはキャッシュから損失へ供給されます。<b>勾配は student だけ</b>に流れ、teacher 整合度は約 <b>0.03 → 0.99</b> まで上がります。</figcaption></figure>
...
**課題:** teacher CLIP を x551 小さい student CNN に埋め込み蒸留し、**ゼロショット精度・検索 Recall@k・teacher 整合度の「保持率」**を一気通貫で評価せよ。蒸留前（ランダム student）との比較表と、棒グラフ `lectures/39_clip_distillation/outputs/mini_project_summary.png` を出力する。

<figure class="lec-fig"><svg viewBox="0 0 640 300" role="img" aria-label="ミニプロジェクトの流れ 合成画像 teacher前計算 student蒸留 3軸評価で保持率を算出" font-family="ui-sans-serif, system-ui, 'Noto Sans JP', sans-serif"><text x="320" y="34" text-anchor="middle" font-size="15" font-weight="700" fill="#18181b">ミニプロジェクト — 蒸留して保持率を一気通貫で評価</text><rect x="36" y="64" width="250" height="72" rx="9" fill="#f4f4f5" stroke="#71717a" stroke-width="2"/><rect x="350" y="64" width="250" height="72" rx="9" fill="#ffedd5" stroke="#c2410c" stroke-width="2"/><rect x="350" y="196" width="250" height="72" rx="9" fill="#fff7ed" stroke="#ea580c" stroke-width="2"/><rect x="36" y="196" width="250" height="72" rx="9" fill="#eff6ff" stroke="#2563eb" stroke-width="2"/><text x="161" y="94" text-anchor="middle" font-size="14" font-weight="700" fill="#3f3f46">① 合成3クラス画像</text><text x="161" y="116" text-anchor="middle" font-size="11.5" fill="#52525b">赤丸・緑四角・青三角（ラベル無し）</text><text x="475" y="94" text-anchor="middle" font-size="14" font-weight="700" fill="#c2410c">② teacher CLIP を凍結 ❄</text><text x="475" y="116" text-anchor="middle" font-size="11.5" fill="#52525b">512次元埋め込みを前計算</text><text x="475" y="226" text-anchor="middle" font-size="14" font-weight="700" fill="#c2410c">③ student CNN へ蒸留</text><text x="475" y="248" text-anchor="middle" font-size="11.5" fill="#52525b">(1−cos)+MSE で学習</text><text x="161" y="226" text-anchor="middle" font-size="14" font-weight="700" fill="#1d4ed8">④ 3軸で評価し保持率を算出</text><text x="161" y="248" text-anchor="middle" font-size="11.5" fill="#52525b">ゼロショット・R@k・整合度→棒グラフ</text><line x1="286" y1="100" x2="344" y2="100" stroke="#71717a" stroke-width="2"/><polygon points="350,100 340,95 340,105" fill="#71717a"/><line x1="475" y1="136" x2="475" y2="190" stroke="#71717a" stroke-width="2"/><polygon points="475,196 470,186 480,186" fill="#71717a"/><line x1="350" y1="232" x2="292" y2="232" stroke="#71717a" stroke-width="2"/><polygon points="286,232 296,227 296,237" fill="#71717a"/></svg><figcaption><b>章末ミニプロジェクト</b>は、<b>① 合成3クラス画像（ラベル無し）→ ② teacher CLIP を凍結して512次元埋め込みを前計算 → ③ student CNN へ <code>(1−cos)+MSE</code> で蒸留 → ④ ゼロショット精度・検索R@k・コサイン整合度の3軸で評価</b>し<b>保持率</b>を算出、という順に流れます。蒸留前（before KD）と並べた比較表と棒グラフ <code>mini_project_summary.png</code> を出力します。</figcaption></figure>
...
<figure class="lec-fig"><svg viewBox="0 0 660 240" role="img" aria-label="次の一手 蒸留studentをONNX量子化しFAISS検索基盤に載せエッジCLIP検索を完成" font-family="ui-sans-serif, system-ui, 'Noto Sans JP', sans-serif"><text x="330" y="40" text-anchor="middle" font-size="15" font-weight="700" fill="#18181b">次の一手 — 蒸留 student をエッジ CLIP 検索へ載せる</text><rect x="24" y="103" width="150" height="86" rx="8" fill="#fff7ed" stroke="#ea580c" stroke-width="2"/><text x="99" y="140" text-anchor="middle" font-size="13.5" font-weight="700" fill="#c2410c">蒸留 student</text><text x="99" y="166" text-anchor="middle" font-size="11.5" fill="#52525b">小さい画像エンコーダ</text><rect x="192" y="103" width="150" height="86" rx="8" fill="#ffedd5" stroke="#c2410c" stroke-width="2"/><text x="267" y="140" text-anchor="middle" font-size="13.5" font-weight="700" fill="#c2410c">ONNX化・量子化</text><text x="267" y="166" text-anchor="middle" font-size="11.5" fill="#52525b">36 / 37 章</text><rect x="360" y="103" width="148" height="86" rx="8" fill="#dbeafe" stroke="#2563eb" stroke-width="2"/><text x="434" y="140" text-anchor="middle" font-size="13.5" font-weight="700" fill="#1d4ed8">FAISS 検索基盤</text><text x="434" y="166" text-anchor="middle" font-size="11.5" fill="#52525b">17 / 42 章</text><rect x="524" y="103" width="134" height="86" rx="8" fill="#fafafa" stroke="#16a34a" stroke-width="2"/><text x="591" y="140" text-anchor="middle" font-size="12.5" font-weight="700" fill="#15803d">エッジ CLIP 検索</text><text x="591" y="166" text-anchor="middle" font-size="11.5" fill="#52525b">完成 ✓</text><line x1="174" y1="146" x2="186" y2="146" stroke="#71717a" stroke-width="2"/><polygon points="192,146 182,141 182,151" fill="#71717a"/><line x1="342" y1="146" x2="354" y2="146" stroke="#71717a" stroke-width="2"/><polygon points="360,146 350,141 350,151" fill="#71717a"/><line x1="508" y1="146" x2="518" y2="146" stroke="#71717a" stroke-width="2"/><polygon points="524,146 514,141 514,151" fill="#71717a"/></svg><figcaption><b>蒸留して得た student</b> は、そのままエッジ検索基盤に載せられます。<b>① 蒸留 student</b>（小さい画像エンコーダ）→ <b>② ONNX 化・動的量子化</b>（36/37 章）→ <b>③ FAISS の検索基盤</b>（17/42 章）→ <b>④ エッジで動く CLIP 検索の完成</b>、という流れです。<b>L2 正規化</b>と <code>logit_scale</code> の約束さえ保てば、teacher のテキスト埋め込みでそのままゼロショット検索ができます。</figcaption></figure>
...
図と重みは `lectures/39_clip_distillation/outputs/` に保存される（matplotlib は Agg、`imshow` は使わない）。初回のみ teacher CLIP の重みを HuggingFace から取得する（以降キャッシュ）。
