...
学習し終えたモデルをデプロイする段になると、目的が「精度を上げる」から「同じ精度を**速く・小さく**動かす」へ変わります。ここで使う刃は大きく3つ。**量子化**は重みや活性を fp32(32bit) から int8(8bit) へ落として「1要素あたりの情報量」を減らす。**枝刈り(pruning)**は重要でない重みを 0 にして「使う重みの数」を減らす。**蒸留**（第38/39回）は小さいモデルに大きいモデルの知識を移す。本章は前2つ、量子化と枝刈りを扱います。どれも「精度を少しだけ犠牲にして、速度とサイズを稼ぐ」取引であり、**どれだけ犠牲を払ったか**を必ず測るのが鉄則です。

量子化の効き目は直感的です。fp32 は 1 要素 4 バイト、int8 は 1 バイト。素朴に考えてモデルは 1/4 に縮みます。さらに int8 の積和は整数演算ユニットで高速に回せるので、対応カーネルがあれば速くもなる。落とし穴は「精度がどれだけ落ちるか」と「**そのカーネルが手元の CPU に本当にあるか**」です。Conv の int8 実行には activation の範囲を事前に知る必要があり（静的PTQ）、Linear だけならその場量子化で足ります（動的PTQ）。この「**何が量子化されるか**」がモデル構造（Conv 主体か Linear 主体か）と噛み合うかで、効果は大きく変わります。

枝刈りはもっと罠が深い。重みの 90% を 0 にすれば 1/10 になりそうですが、**`torch.nn.utils.prune` がするのは「0 のマスクを掛ける」だけ**。テンソルの形も dtype も変わらず、密行列の積（GEMM）がそのまま走ります。0 を掛け算しても計算量は減りません。`prune.remove` はマスクを重みに焼き付けるだけで、やはり密。**疎(sparse)フォーマットに変換すれば「保存サイズ」は縮められますが、CPU には疎行列の高速カーネルがほぼ無いため「実行速度」は縮みません**。本当に速く・小さくしたいなら、チャネルやニューロンを丸ごと落とす**構造化枝刈り**で、小さい密モデルに作り直す必要があります。本章はこの差を `04` で数字で突きつけます。

---

## 2. 理論 — scale / zero-point、対称/非対称、per-channel、PTQ と QAT

int8 量子化は **実数区間 `[x_min, x_max]` を整数区間 `[qmin, qmax]`（int8 なら 0..255 か -128..127）へ線形写像する**ことです。写像は2つの数で決まります。`scale` は「整数 1 段あたりの実数の刻み幅」、`zero_point` は「実数の 0.0 がどの整数に当たるか」。量子化は `q = round(x/scale) + zero_point`（範囲外はクランプ）、逆量子化は `x ≈ scale * (q - zero_point)`。ここで2系統があります。**対称(symmetric)**は `zero_point=0` 固定で `|x|max` から scale を決める方式で、0 を中心に正負対称に分布する**重み**向き。**非対称(asymmetric/affine)**は zero_point を使って片側に寄った分布（ReLU 後の非負な**活性**など）にぴったり合わせられます。`01` では同じ非負ベクトルに両方を当て、非対称の方が MAE が小さくなる（zero_point で 0.0 を整数に正確に置けるから）ことを見ます。

もう一つの軸が **per-tensor か per-channel か**。per-tensor はテンソル全体で scale を1つだけ持つ。per-channel は出力チャネル（重み行列の行）ごとに scale を持つ。チャネル間で重みの振幅が桁違いに違うとき（よくある）、per-tensor だと巨大チャネルに合わせた粗い scale で小振幅チャネルが潰れてしまう。`01` の実験では、行ごとに振幅が 0.01〜1000 倍まで違う重みで per-channel が per-tensor の**約3倍**誤差を下げました。だから実用の重み量子化は **per-channel が既定**です（`get_default_qconfig` もそうなっている）。ビット幅を下げるほど段階数が減って誤差が増える（int8=256段、int4=16段、int2=4段）ことも合わせて押さえます。int8 は randn をほぼ保てる一方、int2 の MAE は int8 の約100倍でした。

**いつ何を測って量子化するか**で PTQ / QAT が分かれます。**動的PTQ(dynamic)**は重みだけ事前に int8 化し、活性は推論のたびにその場で範囲を測って量子化する。キャリブレーション不要・1行で済み、CPU で最も手軽。ただし対象は既定で Linear/LSTM なので **Conv 主体には効きません**。**静的PTQ(static)**は代表データを流して活性の範囲を**事前に観測(キャリブレーション)**し固定する。Conv も int8 実行でき、`fuse_modules`（conv→bn→relu を1演算に畳む）と `QuantStub/DeQuantStub`（量子化の境界）が必要。**QAT**は学習中に **fake-quant**（前向きで丸めを模擬し、逆伝播はストレートスルー）を挟んで、量子化誤差込みで重みを微調整する。PTQ で精度が落ちすぎる時の上位互換で、学習コストを払う代わりに精度を取り戻します。意思決定としては「動的で足りるか → ダメなら静的 → それでも精度が足りなければ QAT」。

---

## 3. 正準 API — `torch.ao.quantization` と `torch.nn.utils.prune`

CPU で確実に動く正準は、本講座では **`torch.ao.quantization`（eager mode）** です。実行すると「将来 torchao へ移行」という `DeprecationWarning` が出ますが、torch 2.12 でも問題なく動き、CPU の int8 実行カーネル（fbgemm/x86/qnnpack）に直結します。新しい `torchao.quantization.quantize_` は現行の推奨 API ですが**任意依存**で、int4/float8 の高速パスは主に GPU/ARM 向け。x86 CPU で確実なのは int8 系なので、本章は torch.ao を主軸にし、torchao は `05` で概念紹介します。
...
QAT は `prepare_qat` 系を使い、**fuse は QAT 用の `fuse_modules_qat`**（train モード対応）で行う点だけ違います（`fuse_modules` を train モードで呼ぶと `Fusion only for eval!` で落ちる、が定番の罠）。`q.qconfig = get_default_qat_qconfig("x86")` → `prepare_qat(q)` → 数エポック微調整 → `q.eval(); convert(q)`。

枝刈りは **`torch.nn.utils.prune`**。非構造化は `prune.l1_unstructured(module, "weight", amount)`（小さい重みから個別に 0）、全層横断は `prune.global_unstructured([...], prune.L1Unstructured, amount)`、構造化は `prune.ln_structured(module, "weight", amount, n=2, dim=0)`（行=出力チャネル単位で落とす）。`prune.remove(module, "weight")` でマスクを焼いて再パラメータ化を外します。**重要なのは、これらはどれも密テンソルのまま**だということ。`module.weight_mask` を見ればマスクが別バッファとして増えているのが分かり、`prune.remove` 前の `state_dict` は `weight_orig + weight_mask` で**むしろ大きくなります**。

---

## 4. 実装を1つずつ — スクリプトで段階的に確かめる

各スクリプトは独立に動き、モデルは合成データで**その場で軽く学習**（CPU で 1〜2 秒・決定的）します。共通部品は `quant_lab.py`（合成タスク・`TinyMLP`/`TinyConvNet`・学習・三角評価の道具・量子化/枝刈りヘルパ）。結果は `outputs/35_quantization_pruning/` に保存されます。
...
`mini_project.py` が完成形です。`CompressionBench` クラスが、**1つのベースモデルに複数の圧縮手法を適用し、accuracy / size(MB) / latency(p50,p99) を同一指標で表にする**ベンチを提供します。
