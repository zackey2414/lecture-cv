...
:root{
  --p50:#faf5ff;--p100:#f3e8ff;--p200:#e9d5ff;--p400:#c084fc;--p500:#a855f7;--p600:#9333ea;--p700:#7e22ce;--p800:#6b21a8;--p900:#581c87;
...
.lang-levels{display:grid;grid-template-columns:repeat(auto-fill,minmax(290px,1fr));gap:1rem;margin-top:1.1rem}
...
.level-badge.intro{background:#16a34a}.level-badge.beginner{background:#2563eb}.level-badge.intermediate{background:#d97706}.level-badge.advanced{background:#dc2626}
...
.nav-track{font-size:.7rem;font-weight:800;color:var(--p700);margin:.75rem .45rem .2rem;letter-spacing:.02em;text-transform:none}
...
/* right: in-page TOC */
.side-title{font-size:.76rem;font-weight:800;color:var(--p700);padding:.3rem .4rem .35rem;letter-spacing:.02em}
...
/* ===== module banner ===== */
.mod-banner{background:linear-gradient(135deg,var(--p800),var(--p600));color:#fff;border-radius:var(--radius);padding:1.7rem 1.8rem;margin-bottom:1.4rem}
...
.prereq-row a{display:inline-block;background:rgba(255,255,255,.2);color:#fff;padding:.1em .5em;border-radius:6px;text-decoration:none;font-size:.86em;margin:.1rem .2rem .1rem 0}
...
.content h3{font-size:1.17rem;color:var(--p700)}
...
.content thead th{background:var(--p100);color:var(--p800);font-weight:700;white-space:nowrap}
.content tbody tr:nth-child(even){background:var(--g50)}
.content :not(pre)>code{background:#f3eafd;color:#9333ea;padding:.12em .42em;border-radius:5px;font-size:.85em;overflow-wrap:anywhere}
...
/* graph helpers */
.mermaid{background:#fff;border:1px solid var(--g200);border-radius:12px;padding:1rem;overflow-x:auto;margin:1.1rem 0;text-align:center}
...
@media(max-width:600px){.hero{padding:3.5rem 1rem 4rem}.cards{margin-top:-2rem;padding:0 1rem}.mod-banner{padding:1.3rem 1.2rem}}
"""
...
<header class="site-header">
  <a class="home-link" href="{rel}index.html">
    ...
    <span class="logo-text">lecture-cv<small>Computer Vision を自分の力にする</small></span>
  </a>
  <nav class="topnav">
