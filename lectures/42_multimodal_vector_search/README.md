...
**意味的に整列した共有空間**ができる。CLIP は softmax 損失、SigLIP は各ペア独立の sigmoid 損失という違いはあるが、
「同じ空間にマップして内積で測る」点は共通で、検索の使い方は同じだ。

検索で使う距離は **コサイン類似度** `cos(a,b) = (a·b)/(‖a‖‖b‖)`。ベクトルの「向き」だけを見て「長さ」を無視するのがミソで、
CLIP の生の出力ベクトルは長さ（ノルム）がバラバラ（本章の実測で約 11 前後）なので、長さに引っ張られると検索が壊れる。
...
「その場で書き換える」ので、生ベクトルを後で使いたいなら **コピーしてから**渡すこと。距離の向きも要注意で、
`IndexFlatIP`（内積）は **大きいほど近い**、`IndexFlatL2`（L2距離）は **小さいほど近い** と、ソート方向が逆になる。
