...
これまでの章は「画像を**理解する**（分類・検出・セグメ・埋め込み）」側でした。本章で CV の理解を
**生成・編集する**側へ拡張します。具体的には、拡散モデル **SD-Turbo** で text-to-image / img2img を
**CPU で数秒**で動かし、**インペイント**（傷消し・物体除去）と**超解像**（古典補間 → Swin2SR）、
**背景除去**（GrabCut）を実装し、最後に **PSNR / SSIM / CLIPScore / FID** で生成・復元の品質を
「正しい向きで」評価できるようになります。
...
乗っているか」を予測しては引く、という**デノイズを反復**して画像へ近づけます。テキスト条件は、各
デノイズステップに「赤いりんご」などの意味ベクトル（CLIP テキスト埋め込み）を注入することで効きます。

通常の Stable Diffusion はこの反復を 20〜50 回まわすため、CPU では 1 枚に数分かかります。本章で使う
...
外から `guidance_scale` をかけると二重適用になって破綻します。だから Turbo では `0.0` が正解です。
...
で十分）。`float32` を明示するのも要点で、CPU では `float16` は遅い・未対応 op があり不安定です。
...
編集モードです。どれだけノイズを足すかが **`strength`**（0=元画像のまま、1=ほぼ作り直し）。SD-Turbo の
...
### 直感
インペイントは「**マスクで指定した領域だけを描き直す**」編集です。傷消し・透かし除去・不要物の除去が
典型用途です。アプローチは大きく 2 つ。古典の **`cv2.inpaint`** は、マスク境界の色・勾配を内側へ
**伝播**させて埋めます（Telea 法＝高速な距離重み伝播、Navier-Stokes 法＝流体方程式に倣った伝播）。
深層の拡散インペイント／LaMa は、マスク内を「周囲＋プロンプトに整合する**新しい内容**」で生成します。
...
```
本章のシーンでは Swin2SR が bicubic を **PSNR で約 +1.5dB** 上回ります。Swin2SR は**ウィンドウ幅の倍数に内部
パディング**するので出力が厳密に 2×H,2×W にならないことがあり、目標サイズへクロップ/リサイズして揃えます
