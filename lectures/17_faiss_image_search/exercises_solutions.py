"""第17回 演習の模範解答ランナー（全問 PASS）。

exercises.py の TODO をすべて埋めた状態を再現する。重複を避けるため、
採点ロジック(_grade)も模範解答(_sol_*)も exercises 側のものをそのまま再利用し、
ここでは「解答を差し込んで採点を回すだけ」に徹する。

実行:
  uv run python lectures/17_faiss_image_search/exercises_solutions.py
  → 10/10 PASS（ALL PASS）になることを確認する。
